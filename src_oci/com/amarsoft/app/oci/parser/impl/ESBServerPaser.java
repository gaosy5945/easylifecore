 ...
 * <p>ESB组包拆包处理</p>
 */
public class ESBServerPaser extends BasicParser {
	...
	public void compositeTransData(OCITransaction trans) throws Exception {
		...
		for(String key:trans.getOMessages().keySet())
		{
			...
			{
				//设置Header
				Message header = trans.getOMessage(key);
				...
				//设置Body
				Message body = trans.getOMessage(key);
		...
		trans.setResponseData(envelope);
	}
	...
	public void decomposeTransData(OCITransaction trans) throws Exception {
		SOAPEnvelope envelope = (SOAPEnvelope)trans.getRequestData();
		...
		for(String key:trans.getIMessages().keySet())
		{
			...
			{
				//设置Header
				Message message = trans.getIMessage(key);
