...
<%
	...
	if(sBookType.equals("040")&&"01".equals(sLawCaseType))
	{
		...
		if(sSql.equals("0")) sAddFlag="立案台账受理结果为‘受理’时才可以新增一审台账！";
		else  sAddFlag="success";
	}
	if(sBookType.equals("026")&&"01".equals(sLawCaseType))
	{
		sSql=Sqlca.getString("select count(*) from lawcase_book lb where lb.booktype='030' and lb.CognizanceResult='1001' and LAWCASESERIALNO='"+sSerialNo+"' ");
		if(sSql.equals("0")) sAddFlag="立案台账受理结果为‘受理’时才可以新增诉讼保全台账！";
		else  sAddFlag="success";
	}
	if(sBookType.equals("050")&&"01".equals(sLawCaseType))
	{
		...
		if(sSql.equals("0")) sAddFlag="一审台账录入一审结果才可以新增二审台账！";
		else sAddFlag="success";
	}
	if(sBookType.equals("060")&&"01".equals(sLawCaseType))
	{
		...
		if(sSql.equals("0")) sAddFlag = "一审或二审台账录入判决书生效日期才可以新增再审台账！";
		else sAddFlag = "success";
	}
	if(sBookType.equals("070")&&"01".equals(sLawCaseType))
	{
		...
		if(sSql.equals("0"))sAddFlag = "一审台账录入判决书生效日期或二审台账录入判决书生效日期或支付令台账录入支付令裁定日期才可以新增执行台账！";
		else sAddFlag = "success";
	}
	if(sBookType.equals("070")&&"02".equals(sLawCaseType))
	{
		sSql=Sqlca.getString("select count(*) from lawcase_book lb where ("+
				            ...
				            ") and  LAWCASESERIALNO='"+sSerialNo+"' ");
		if(sSql.equals("0"))sAddFlag = "公证台账录入公证书下达日期或仲裁台账录入仲裁结果才可以新增执行台账！";
		else sAddFlag = "success";
	}
...
<%/*~BEGIN~可编辑区~[Editable=true;CodeAreaID=List04;Describe=定义按钮;]~*/%>
	...
	//依次为：
		...
		//6.资源图片路径
	String sButtons[][] = {
			{sFinishType.equals("")?(sPigeonholeDate.equals("")?"true":"false"):"false","All","Button","新增","新增一条记录","newRecord()","","","",""},
			{"true","","Button","详情","查看/修改详情","viewAndEdit()","","","",""},
			{sFinishType.equals("")?(sPigeonholeDate.equals("")?"true":"false"):"false","All","Button","删除","删除所选中的记录","deleteRecord()","","","",""}		
		};
	%> 
<%/*~END~*/%>
