...
<%
	...
	sSql =  " select AppDate,AcceptedCourt,AcceptedDate,AcceptedNo "+
	        " from LAWCASE_BOOK "+
	        " where LawCaseSerialNo =:ObjectNo "+ 
	        " and (BookType='026' "+
	        " or BookType='030' "+
	        " or BookType='040' "+
	        " or BookType='050' "+
	        ...
	        " or BookType='065' "+
	        " or BookType='068')) ";
	so = new SqlObject(sSql).setParameter("ObjectNo",sObjectNo).setParameter("ObjectNo",sObjectNo);
...
<%/*~BEGIN~可编辑区~[Editable=true;CodeAreaID=Info03;Describe=定义数据对象;]~*/%>
	...
	if (sBookType.equals("010")){	//支付令台帐
		...
		sTempletNo="DamageInfo1";
		sItemdescribe="10";		//根据不同的台帐类型过滤受理结果
	}else if (sBookType.equals("030")){	//立案台帐
		sTempletNo="BeforeLawsuitInfo";
		sItemdescribe="10";
	//}else if (sBookType.equals("040")){	//受审台账 
	//	sTempletNo="OnTrialLawsuitInfo";
	//	sItemdescribe="20";
	...
	//定义自动累计字段 getSuitJudgeSum  getActualSum
	if ((sBookType.equals("040"))||(sBookType.equals("050"))||(sBookType.equals("060"))){
		doTemp.appendHTMLStyle("ConfirmPI,ConfirmFee,LawyerFee,OtherFee"," onChange=\"javascript:parent.getSuitJudgeSum()\" ");
		doTemp.appendHTMLStyle("ActualPrincipal,ConfirmFee,ActualOutInterest,ActualOtherSum"," onChange=\"javascript:parent.getActualSum()\" ");
	...
	if(!(sBookType.equals("068"))){
		//选择主承办法官
		doTemp.setUnit("WithdrawProposerName"," <input type=button class=inputDate  value=... name=button onClick=\"javascript:parent.getAgentName1()\">");
		doTemp.appendHTMLStyle("WithdrawProposerName","  style={cursor:pointer;background=\"#EEEEff\"} ondblclick=\"javascript:parent.getAgentName1()\" ");
	}
	...
	//生成HTMLDataWindow
	Vector vTemp = dwTemp.genHTMLDataWindow(sObjectNo+","+sSerialNo);	
...
<%/*~BEGIN~可编辑区~[Editable=false;CodeAreaID=Info06;Describe=定义按钮事件;]~*/%>
	...
	function saveRecord(sPostEvents)
	{
		...
		if (sBookType == "040" || sBookType == "050")	//一审、二审台帐
		{
			//获得受理结果、判断金额、判决/调整日期
			var sCognizanceResult = getItemValue(0,getRow(),"CognizanceResult");
			var sJudgeSum = getItemValue(0,getRow(),"JudgeSum");
			var sJudgedDate = getItemValue(0,getRow(),"JudgedDate");
			
			//如果一审结果录入后，判决金额以及判决/调整日期必输
			if(sCognizanceResult != "" && ((sJudgedDate == "" || sJudgedDate.length == 0) || (sJudgeSum == "" || sJudgeSum.length == 0)))
			{
				 
				 alert("受理结果录入后，判决金额以及判决/调整日期必输");
				 return;
			}
		}
		if (sBookType == "020")	//诉前保全台账
		{
			//获得是否正式上诉
			var sRetrialPhase = getItemValue(0,getRow(),"retrialphase");
			var sApplicant = getItemValue(0,getRow(),"applicant");
			//如果“是否正式起诉”若选择否，则“不起诉原因”必输
			if(sRetrialPhase == 0 && sRetrialPhase != "" && sApplicant == "")
			{
				 alert("“是否正式起诉”若选择否，则“不起诉原因”必输");
				 return;
			}
		}
		/* if (sBookType == "040" || sBookType == "050" || sBookType == "060")	//受审台账
		{
			var sAppDate = getItemValue(0,getRow(),"AppDate");//申请日期
			var sAcceptedDate  = getItemValue(0,getRow(),"AcceptedDate");//受理日期
			var sApproveDate  = getItemValue(0,getRow(),"ApproveDate");//开庭日期
			if(!compareDate(sAppDate,sAcceptedDate,3)){
				alert("申请日期应该早于受理日期！");
				return;
			}
			if(!compareDate(sAcceptedDate,sApproveDate,3)){
				alert("受理日期应该早于开庭日期！");
				return;
			}
		} */
...
<%/*~BEGIN~可编辑区~[Editable=false;CodeAreaID=Info07;Describe=自定义函数;]~*/%>
	...
	//选择仲裁机构
  	function getAcceptedCourtName(){
  		//AsDialog.SetGridValue("CourtList", "DepartType=02", "AcceptedCourt=AgencyName", "", "");
	...
	/*~[Describe=选择审理法院名称;InputParam=无;OutPutParam=无;]~*/
	function getAcceptedCourt(){
	...
	/*~[Describe=选择受理名称;InputParam=无;OutPutParam=无;]~*/
	function getWriteAgent(){
		//CourtPersonList
		var sValue = AsDialog.SelectGridValue("CourtPersonList", "", "CompromiseBrief@AGENTNAME", "", "");
		setItemValue(0,0,"CompromiseBrief",sValue.split("@")[1]);	
	}
	...
	/*~[Describe=选择主承办法官;InputParam=无;OutPutParam=无;]~*/
	function getAgentName1(){
		AsDialog.SetGridValue("CourtPersonList", "", "WithdrawName=AGENTNAME", "", "");
	}
	/*~[Describe=选择代理人1;InputParam=无;OutPutParam=无;]~*/
	...
	/*~[Describe=选择代理人2;InputParam=无;OutPutParam=无;]~*/
	function getCurrentAgent1(){
		sParaString = "AgentType"+",02";
		var sReturn = setObjectValue("SelectAgent",sParaString,"",0,0,"");
		if (!(sReturn == '_CANCEL_' || typeof(sReturn) == "undefined" || sReturn.length == 0 || sReturn == '_CLEAR_')){
			sReturn = sReturn.split("@");		
			...
			setItemValue(0,0,"BANKRUPTCYORGNAME",sAgentName);
		}else if (sReturn=='_CLEAR_'){
			setItemValue(0,0,"BANKRUPTCYORGNAME","");
		}else{
			return;
		}
	}
	/*~[Describe=仲裁台账--选择外聘代理人;InputParam=无;OutPutParam=无;]~*/
	function getAgentWithAgentType(AgentType){
		var sAgentType ="";
		if(AgentType==2){
			sAgentType = "02";
		}else if(AgentType==1){
		...
		sParaString = "AgentType,"+sAgentType;
		var sReturn = setObjectValue("SelectAgent1",sParaString,"",0,0,"");
		if (!(sReturn == '_CANCEL_' || typeof(sReturn) == "undefined" || sReturn.length == 0 || sReturn == '_CLEAR_')){
			sReturn = sReturn.split("@");		
			//代理人名称、代理人所属机构
			var sAgentName = sReturn[1];
			...
			setItemValue(0,0,"CurrentAgent","");
			setItemValue(0,0,"WithdrawName","");
		}else{
			return;
		}
	}
	//JointCourt  选择合议庭成员
	function getJointCourt(){
		var sValue = AsDialog.SelectGridValue("CourtPersonList", "", "JointCourt@AGENTNAME", "true", "","","","");
		setItemValue(0,0,"JointCourt",sValue.split("@")[1]);	
	}
	...
	function initRow(){
		if (getRowCount(0)==0)
		{
			...
			<%-- //modify by lzq 20150331 改为不需要默认
			var sBookType = "<%=sBookType%>";			
	...
	function getActualSum(){ 
 	...
 	//根据输入的其中：本金，表内利息，表外利息，其它算出 执行台账 申请执行总标的  以及  未清偿本金余额（元）
	function getApplySum(){ 
		var sApplyPrincipal = getItemValue(0,getRow(),"ApplyPrincipal");
		var sApplyInInterest = getItemValue(0,getRow(),"ApplyInInterest");
		var sApplyOtherSum = getItemValue(0,getRow(),"ApplyOtherSum");
		...
		//诉讼总标的=其中：本金+表内利息+表外利息+其它
		var sApplySum = sApplyPrincipal+sApplyInInterest+sApplyOtherSum;
	
     	setItemValue(0,getRow(),"ApplySum",sApplySum);
 	}
 	//根据输入的其中：本金，表内利息，表外利息，其它算出 执行台账 申请执行总标的  未清偿本金余额（元）
	function getSuitApplySum(){ 
		var sApplyPrincipal = getItemValue(0,getRow(),"ApplyPrincipal");
		var sApplyInInterest = getItemValue(0,getRow(),"ApplyInInterest");
		var sApplyOtherSum = getItemValue(0,getRow(),"ApplyOtherSum");
		...
		var sActualSum = getItemValue(0,getRow(),"ActualSum");
		
     	setItemValue(0,getRow(),"ApplySum",sApplySum);
     	setItemValue(0,getRow(),"ActualOutInterest",sApplySum - sActualSum);
 	}

 	//根据输入的其中：本金，利息，其它算出执行收回总金额	
	function getSuitActualSum(){ 
		sActualPrincipal = getItemValue(0,getRow(),"ActualPrincipal");
		sActualInInterest = getItemValue(0,getRow(),"ActualInInterest");
		sActualOtherSum = getItemValue(0,getRow(),"ActualOtherSum");
		...
		sApplySum = getItemValue(0,getRow(),"ApplySum");
 	...
 	//根据输入的其中：本金，利息，其它算出判决金额(RMB元：以下四项合计)
	function getSuitJudgeSum(){ 
		ConfirmPI = getItemValue(0,getRow(),"ConfirmPI");
		ConfirmFee = getItemValue(0,getRow(),"ConfirmFee");
		LawyerFee = getItemValue(0,getRow(),"LawyerFee");
		OtherFee = getItemValue(0,getRow(),"OtherFee");
		
		sCurrency = getItemValue(0,getRow(),"Currency");
		
		if(typeof(ConfirmPI)=="undefined" || ConfirmPI.length==0) ConfirmPI=0.0; 
		if(typeof(ConfirmFee)=="undefined" || ConfirmFee.length==0) ConfirmFee=0.0;
		if(typeof(LawyerFee)=="undefined" || LawyerFee.length==0) LawyerFee=0.0; 
		if(typeof(OtherFee)=="undefined" || OtherFee.length==0) OtherFee=0.0; 
    
		sJudgeSum = parseFloat(ConfirmPI) + parseFloat(ConfirmFee) + parseFloat(LawyerFee) + parseFloat(OtherFee);

     	setItemValue(0,getRow(),"JudgeSum",sJudgeSum);
 	}
	</script>
