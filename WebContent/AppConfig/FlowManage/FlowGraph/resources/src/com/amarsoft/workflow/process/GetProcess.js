...
//
GetProcess.prototype.getProcess = function (name,version) {
    if (!name) {
        this.name = null;
    	//加载失败，工作流程图名字为空！
    ...
    var method = "POST";
    var params = "name=" + name+ "&version="+version;
...
//
GetProcess.prototype.getProcessInst = function (name, curacts, version) {
    if (!name) {
        this.name = null;
    	//加载失败，工作流程图名字为空！
        alert("\u52a0\u8f7d\u5931\u8d25\uff0c\u5de5\u4f5c\u6d41\u7a0b\u56fe\u540d\u5b57\u4e3a\u7a7a\uff01");
    ...
    var url = WorkFlowWorkSpace.URL_GET_PROCESS;
    var method = "POST";
    var params = "name=" + name + "&curActs=" + curacts + "&version=" + version;
