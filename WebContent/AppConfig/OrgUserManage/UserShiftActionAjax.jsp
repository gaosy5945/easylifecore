 ...
 */
	...
	try{
		...
		sSql =  " update CLASSIFY_RECORD set CLASSIFYORGID = :OrgID where CLASSIFYUSERID = :UserID ";
		so = new SqlObject(sSql);
		so.setParameter("OrgID",sToOrgID);
		so.setParameter("UserID",sFromUserID);
		Sqlca.executeSQL(so);		
		...
		sSql =  " update DOC_LIBRARY set InputOrgID = :InputOrg,UpdateTime = :UpdateTime where InputUserID = :InputUser ";
		so = new SqlObject(sSql);
		so.setParameter("InputOrg",sToOrgID);
		so.setParameter("UpdateTime",StringFunction.getToday());
		so.setParameter("InputUser",sFromUserID);
		...
		sSql =  " update FLOW_OBJECT set OrgID = :OrgID where UserID = :UserID ";
		so = new SqlObject(sSql);
		so.setParameter("OrgID",sToOrgID);
		so.setParameter("UserID",sFromUserID);
		Sqlca.executeSQL(so);
		
		sSql =  " update FLOW_TASK set OrgID = :OrgID where UserID = :UserID ";
		so = new SqlObject(sSql);
		so.setParameter("OrgID",sToOrgID);
		so.setParameter("UserID",sFromUserID);
		Sqlca.executeSQL(so);
		...
		sSql =  " update AWE_ROLE_INFO set InputOrg = :InputOrg,UpdateTime = :UpdateTime where InputUser = :InputUser ";
		so = new SqlObject(sSql);
		so.setParameter("InputOrg",sToOrgID);
		so.setParameter("UpdateTime",StringFunction.getToday());
		so.setParameter("InputUser",sFromUserID);
