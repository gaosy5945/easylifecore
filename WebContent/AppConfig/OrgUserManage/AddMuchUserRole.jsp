...
<%
	...
	/* String sSql = " select RoleID || '   ' || RoleName as Allrole,RoleID" +
		   " from AWE_ROLE_INFO readpast "+
		   " where length(RoleID)>1 ";//orcal */
	...
	/* String sSql1 = " select UserID|| '   ' || UserName as AllUser,UserID" +
			" from USER_INFO readpast where BelongOrg = :BelongOrg ";//orcal */
	String sSql1 = " select concat(UserID , ' ' ,UserName) as AllUser,UserID" +
			" from USER_INFO readpast where BelongOrg = :BelongOrg ";//mysql
   ...
   function pushButton(buttonName,push){
	...
	function doQuery(){
		if(confirm(getMessageText("ALS70906"))){ //您确定给所选的所有用户新增所选的所有角色吗？
    		...
    		var sReturnMessage = RunJavaMethodTrans("com.amarsoft.app.awe.config.orguser.action.AddMuchRolesAction","addMuchRoles","UserID=<%=CurUser.getUserID()%>,Action=Add,UsersID="+vReturn1+",RolesID="+vReturn);
	...
	function doDelete(){
        if(confirm(getMessageText("ALS70909"))){ //您确定要删除所选用户中您所选的所有角色吗？
    		...
    		var sReturnMessage = RunJavaMethodTrans("com.amarsoft.app.awe.config.orguser.action.AddMuchRolesAction","addMuchRoles","UserID=<%=CurUser.getUserID()%>,Action=Del,UsersID="+vReturn1+",RolesID="+vReturn);
			alert(sReturnMessage);
			top.close();
        }     
	}
