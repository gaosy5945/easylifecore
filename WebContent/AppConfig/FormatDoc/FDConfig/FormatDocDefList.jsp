<%@ page contentType="text/html; charset=GBK"%><%@
 include file="/Frame/resources/include/include_begin_list.jspf"%><%
	...
	String sButtons[][] = {
		...
		{"true","","Button","验证规则","验证规则","validatorDialog()","","","",""},
		{"true","","Button","字段中文映射","字段中文映射配置","nameMap()","","","",""},
	};
...
<script type="text/javascript">
	setDialogTitle("报告节点定义");
 	...
 	function preview_model(){
		if(getRow()==-1){
	 	...
	 	if(sUrl.substring(0, 1) != "/") sUrl = "/" + sUrl;
	 	AsControl.OpenPage(sUrl,'','');
 	}
 	//下载模板文件
	...
	//验证规则
 	function validatorDialog(){
	 	...
	 	AsControl.PopView("/AppConfig/FormatDoc/FDConfig/ValidateEditFrame.jsp","Dono="+sDono,"");
 	}
 	function nameMap(){
	 	var sDirID = getItemValue(0,getRow(),'DirID');
	 	if(typeof(sDirID) == "undefined" || sDirID.length == 0){
			alert('没有选中可用的行');
		 	return;
	 	}
	 	AsControl.PopView("/AppConfig/FormatDoc/FDConfig/FormatDocNameMap.jsp","DocID=<%=sDocID%>&DirID="+sDirID,"");
 	}
</script>
