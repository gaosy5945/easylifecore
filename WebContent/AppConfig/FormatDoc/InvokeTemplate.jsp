...
<script type="text/javascript" src="<%=sWebRootPath%>/Frame/page/resources/ckeditor/ckeditor/ckeditor.js?t=B37D54V"></script>
...
<%
	if( sMethod.equals(IFormatDocData.FDDATA_DISPLAY)){
		out.println("<table align=\"center\" style=\"width:662px;table-layout:fixed;background:url("+fconfig.getHttpRootPath() + oData.getWatermark() +")\"><tr><td style=\"white-space:nowrap;overflow:hidden;\">" + oData.getHtml(sMethod,"GBK") + "</td></tr></table>");//获得HTML
		...
		try{
			...
			out.println("<table align=\"center\" style=\"width:662px;table-layout:fixed;background:url("+fconfig.getHttpRootPath() + oData.getWatermark() +")\"><tr><td style=\"white-space:nowrap;overflow:hidden;\">" + oData.getHtml(IFormatDocData.FDDATA_DISPLAY,"GBK") + "</td></tr></table>");//获得HTML
			result = "刷新form成功！";
		}catch(Exception e){
...
<%	
	...
	try{
		for (var iEle = 0; iEle < iTextareaLength; iEle++ ){
 			//editor_generate(aTextarea[iEle]);//旧版富文本编辑器
			ckeditor_generate(aTextarea[iEle]);//新版富文本编辑器
