...
function calcNewLoanRateTermID(){
	...
	var obj = $('#ContentFrame_NEWRatePart');
	if(typeof(obj) == "undefined" || obj == null) return;
	...
	OpenComp("BusinessTermView","/Accounting/LoanDetail/LoanTerm/BusinessTermView.jsp","Status=0@1&termMonth="+termMonth+"&ToInheritObj=y&ObjectNo="+documentSerialNo+"&ObjectType="+documentType+"&TempletNo=RateSegmentView&TermObjectType=jbo.acct.ACCT_RATE_SEGMENT&TermID="+sLoanRateTermID,"NEWRatePart","");
}

/*~[Describe=原利率信息;InputParam=无;OutPutParam=无;]~*/
function calcOldLoanRateTermID(){
	var obj = $('#ContentFrame_OLDRatePart');
	if(typeof(obj) == "undefined" || obj == null) return;
	...
	var sMaturityDate = getItemValue(0,getRow(),"LoanMaturityDate");
	var termMonth = RunMethod("BusinessManage","GetUpMonths",sPutoutDate+","+sMaturityDate);
	OpenComp("BusinessTermView","/Accounting/LoanDetail/LoanTerm/BusinessTermView.jsp","RightType=ReadOnly&Status=2&termMonth="+termMonth+"&ObjectNo="+documentSerialNo+"&ObjectType="+documentType+"&TempletNo=RateSegmentView&TermObjectType=jbo.acct.ACCT_RATE_SEGMENT&TermID="+sOLDLoanRateTermID,"OLDRatePart","");
}
