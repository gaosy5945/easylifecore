...
function changePrincipalAmt(){
	...
	if("010"==sChangePrincipalType){
		...
		setItemValue(0,0,"NewOverdueBalance",getItemValue(0,getRow(),"OldOverdueBalance") + sChangePrincipalAmt);
	}else if("020"==sChangePrincipalType){
		var sOldNormalBalance = getItemValue(0,getRow(),"OldNormalBalance");
		var sOldOverdueBalance = getItemValue(0,getRow(),"OldOverdueBalance");
		var sChangePrincipalAmt = getItemValue(0,getRow(),"ChangePrincipalAmt");
		var Sum = sOldNormalBalance + sChangePrincipalAmt;
		if(sChangePrincipalAmt > sOldOverdueBalance){
			alert("调整金额过大，请重新输入！");
			setItemValue(0,0,"ChangePrincipalAmt",0);
			return;
		}
		...
		//setItemValue(0,0,"OldOverdueBalance",sLoanNormalBalance-sOldNormalBalance);
		setItemValue(0,0,"NewOverdueBalance",sOldOverdueBalance - sChangePrincipalAmt);
	}
}
...
function selectOLDAccountCode(id,name)
{
	...
	setObjectValue("SelectOLDAccountCode","LoanSerialNo,"+sLoanSerialNo+",ObjectType,jbo.acct.ACCT_LOAN","@"+id+"@0@"+name+"@1",0,0,"");
}
