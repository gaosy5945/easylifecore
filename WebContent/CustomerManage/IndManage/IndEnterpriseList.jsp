...
<%@ include file="/IncludeBegin.jsp"%><%
	...
	*/
	...
	String sButtons[][] = {
		...
		{"false","","Button","引入","引入已存在的客户","importCustomer()","","","",""},
	};
...
<script type="text/javascript">
	...
	function newRecord(){
		...
		nationCode = sReturnValue[4];
	
		//检查客户信息存在状态
		//参数格式：CustomerType,CustomerName,CertType,CertID,UserID
		sReturnStatus = RunJavaMethodTrans("com.amarsoft.app.lending.bizlets.CheckCustomerAction","checkCustomerAction","customerType="+sCustomerType+",customerName="+sCustomerName+",certType="+sCertType+",certID"+sCertID+",userID=<%=CurUser.getUserID()%>");
		//得到客户信息检查结果和客户号
		...
		if(sStatus == "01"){				
			...
			var sParam = ("customerID="+sCustomerID+",customerName="+sCustomerName+",customerType="+sCustomerType+",certType="+sCertType+",certID="+sCertID+",nationCode="+nationCode);
			sParam += (",status="+sStatus+",customerOrgType="+sCustomerOrgType+",userID=<%=CurUser.getUserID()%>,orgID=<%=CurUser.getOrgID()%>");
			//新增客户信息String CustomerID,String CustomerName,String CustomerType,String CertType,String CertID,String NationCode,String Status,String CustomerOrgType,String UserID,String OrgID,String HaCustomerType
			sReturn = RunJavaMethodTrans("com.amarsoft.app.lending.bizlets.AddCustomerAction","addCustomerAction",sParam);
			//已经新增客户
			if(sReturn == "1"){
