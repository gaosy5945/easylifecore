...
/* 	if("03".equals(sCustomerType)){
		doTemp.appendJboWhere(" and O.AddressType in('01','02','03','E01')");
	}else{
		doTemp.appendJboWhere(" and O.AddressType in('04','05','08','E01')");
	} */
	/*
	add by bhxiao modify by 缺陷：2332 ,贷后变更也显示所有的地址信息
	if(addressType != null && !"".equals(addressType))
	{
...
<script type="text/javascript">
	function add(){
		AsControl.OpenPage("/CustomerManage/CustomerAddressInfo.jsp","CustomerID=<%=sCustomerID%>"+"&CustomerType=<%=sCustomerType%>"+"&ListType=<%=sListType%>","_self","");
        reloadSelf();	
	}
	...
	function edit(){
		...
		AsControl.OpenPage("/CustomerManage/CustomerAddressInfo.jsp","SerialNo="+serialNo+"&CustomerID=<%=sCustomerID%>"+"&CustomerType=<%=sCustomerType%>"+"&ListType=<%=sListType%>","_self","");
        reloadSelf();	
	}
