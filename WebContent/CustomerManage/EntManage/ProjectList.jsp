...
<%/*~BEGIN~可编辑区~[Editable=true;CodeAreaID=List04;Describe=定义按钮;]~*/%>
	...
	String sButtons[][] = {
		{"true","All","Button","新增","新增项目信息","newRecord()","","","",""},
		{"true","All","Button","引入","引入项目","doImport()","","","",""},
		{"true","","Button","详情","查看项目详情","viewAndEdit()","","","",""},
		{"true","All","Button","删除","删除项目信息","deleteRecord()","","","",""},
		};
...
<%/*~BEGIN~可编辑区~[Editable=false;CodeAreaID=List06;Describe=自定义函数;]~*/%>
	...
	{   
		...
		{
			sReturn = RunJavaMethodTrans("com.amarsoft.app.lending.bizlets.GetColValue","getColValue","colName=ProjectNo,tableName=PROJECT_RELATIVE,whereClause=String@ObjectNo@" + "<%=sObjectNo%>" + "@String@ObjectType@" + "<%=sObjectType%>" + "@String@ProjectNo@" + sProjectNo);
			...
			{			
				...
				//新增业务与所选项目的关联关系
				sReturn = RunJavaMethodTrans("com.amarsoft.app.bizmethod.ProjectManage","insertProjectRelative","paras=objectNo@@<%=sObjectNo%>"+"@~@objectType@@"+"<%=sObjectType%>"+"@~@projectNo@@"+sProjectNo+"@~@tableName@@PROJECT_RELATIVE");
				if(typeof(sReturn) != "undefined" && sReturn != "")
				{
