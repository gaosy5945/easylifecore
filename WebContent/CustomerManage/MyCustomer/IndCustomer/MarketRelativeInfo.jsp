...
<%
	...
	String sTempletNo = "MarketRelativeInfo";//--模板号--
	ASObjectModel doTemp = new ASObjectModel(sTempletNo);	
	if("01".equals(sCustomerType)){
		doTemp.setDefaultValue("RelationShip", "3095");
	}
	...
	}else if("03".equals(sCustomerType) && !"2".equals(CustomerFlag)){//个人客户中关联人下拉框
		doTemp.setDDDWJbo("RelationShip","jbo.sys.CODE_LIBRARY,itemNo,ItemName,codeno = 'CustomerRelationShip' and ItemNo in ('2001','2002','2003','2004','2005','2007','9999')");
		doTemp.setDDDWJbo("CERTTYPE","jbo.sys.CODE_LIBRARY,itemNo,ItemName,codeno = 'CustomerCertType' and length(itemno) = 1 and isinuse = '1' order by SortNo ");
	}else{//个人客户中关联企业下拉框
		doTemp.setDDDWJbo("RelationShip","jbo.sys.CODE_LIBRARY,itemNo,ItemName,codeno = 'CustomerRelationShip' and isinuse='1' ");
		doTemp.setDDDWJbo("CERTTYPE","jbo.sys.CODE_LIBRARY,itemNo,ItemName,codeno = 'CustomerCertType' and isinuse = '1' order by SortNo ");
	...
	doTemp.setReadOnly("RELATIVECUSTOMERNAME,CERTTYPE,CERTID",true);
...
/*~[Describe=OnChange事件;InputParam=无;OutPutParam=无;]~*/
	function changeRelativeOption(){
 	var sRelationShip = getItemValue(0,0,"RELATIONSHIP");
	if(sRelationShip == "3095"){
		setItemDisabled(0,0,"RELATIVECUSTOMERNAME","true");
		setItemDisabled(0,0,"CERTTYPE","true");
		setItemDisabled(0,0,"CERTID","true");
	}else{
		setItemDisabled(0,getRow(),"RELATIVECUSTOMERNAME","");
		setItemDisabled(0,getRow(),"CERTTYPE","");
		setItemDisabled(0,getRow(),"CERTID","");
	}
}

/*~[Describe=选择关联人;InputParam=无;OutPutParam=无;]~*/
 	function selectMarketCustomer(){
 		var sRelationShip = getItemValue(0,0,"RELATIONSHIP");
 		if(sRelationShip == "3095"){
 			AsCredit.setGridValue("MarketRelativeCustomerList","<%=CurUser.getUserID()%>",'RELATIVECUSTOMERNAME=CUSTOMERNAME@CERTTYPE=CERTTYPE@CERTID=CERTID','','',0,0);
 		}
}
...
/*~[Describe=保存;InputParam=后续事件;OutPutParam=无;]~*/
	...
	{	
		...
		if(!checkCertInfo()) return ;
			...
			if(sResult[0] == "No"){//如果关联客户在本行为一个新客户时，新增该客户，并将其于本客户关联
				...
				if(relationShip == "2007"){
					var sReturn = CustomerManage.judgeIsLover("<%=sCustomerID%>");
					if(sReturn == "2"){
						if(confirm("该客户已存在配偶，是否继续新增配偶？")){
							AddCustomerInfo();
				...
				if(RelativeCustomerID == customerid){
					alert("不允许将客户本身作为关联人！");
					return;
				}else{
					...
					if(sReturn[0] == "2"){
						...
						if((RelationShip == "2007") && (sReturn[1] == "2")){
							if(confirm("该客户已存在配偶，是否继续新增配偶？")){
	...
	function selectCustomer1(){
		AsDialog.SetGridValue("SelectRelativeCustomer", "<%=CurUser.getUserID()%>,<%=sCustomerID%>", "RELATIVECUSTOMERNAME=CUSTOMERNAME@CertType=CERTTYPE@CertID=CERTID@@IssueCountry=ISSUECOUNTRY", "");
	}
