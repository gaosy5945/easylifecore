<%@ page contentType="text/html; charset=GBK"%><%@
 include file="/IncludeBegin.jsp"%><%
	...
	//定义Treeview
	HTMLTreeView tviTemp = new HTMLTreeView(SqlcaRepository,CurComp,sServletURL,"客户分组","right");
	tviTemp.TriggerClickEvent=true; //是否自动触发选中事件

	//参数从左至右依次为：
	//ID字段(必须),Name字段(必须),Value字段,Script字段,Picture字段,From子句(必须),OrderBy子句,Sqlca
	tviTemp.initWithSql("SerialNo","TagName","SerialNo","","","from OBJECT_TAG_CATALOG where SUBSCRIBERTYPE = '01' and SUBSCRIBERID='"+UserID+"'","Order By SerialNo",Sqlca);
	String sButtons[][] = {
...
function startMenu(){
	...
	<%/*[Describe=新增记录;]*/%>
	function newRecord(){
        OpenPage("/CustomerManage/MyCustomerInfo.jsp","frameright","");
        reloadSelf();
	}
	...
	function TreeViewOnClick(){
		...
		if(!sSerialNo){
			OpenPage("/AppMain/Blank.jsp?TextToShow=请选择左边树图节点!", "frameright");
	      	...
	      	OpenPage("/CustomerManage/MyCustomerTagList.jsp?SerialNo="+sSerialNo, "frameright"); 
		}
	}
