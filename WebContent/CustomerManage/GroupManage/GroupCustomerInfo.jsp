...
<%
    ...
    //add by wmzhu 2014/05/19
    //由于集团客户新增后会往Group_Info与Customer_Info表同时插入记录，要保持两边的编号一致
    //客户编号必须要向Customer_Info表内取，不然往Customer_Info内插数据时会造成主键冲突。
	if(sGroupID==null || sGroupID.equals("")) sGroupID=DBKeyHelp.getSerialNo("CUSTOMER_INFO", "CUSTOMERID");
...
<%/*~BEGIN~可编辑区~[Editable=true;CodeAreaID=Info04;Describe=定义按钮;]~*/%>
	...
	//依次为：
		...
		//6.资源图片路径
    String sButtons[][] = {
			{(sRightType.equals("ReadOnly")?"false":"true"),"All","Button","保存","保存","saveRecord('afterEvent()')","","","",""},
			{"false","","Button","集团核心企业信息","集团核心企业信息","viewParent()","","","",""},
			{"false","","Button","取消","取消","cancel()","","","",""}
	};
	
	//具有主办权的用户可修改本页面。
/* 	String sRight = Sqlca.getString(new SqlObject(" select BelongAttribute2 from CUSTOMER_BELONG where CustomerID = :CustomerID and UserID = :UserID ").setParameter("CustomerID",sGroupID).setParameter("UserID",CurUser.getUserID()));
	if("1".equals(sRight)){
		sButtons[0][0] = "true";
	}else{
		sButtons[0][0] = "false";
...
<%/*~BEGIN~可编辑区~[Editable=false;CodeAreaID=Info06;Describe=定义按钮事件;]~*/%>
	...
	function afterEvent(){
		...
		AsCredit.openFunction("CustomerDetail","CustomerID="+sGroupID,""); 
		//AsControl.OpenObject("Customer",sGroupID,"001");
		cancel();
	}
	function cancel(){
		top.close();
...
<%/*~BEGIN~可编辑区~[Editable=false;CodeAreaID=Info07;Describe=自定义函数;]~*/%>
	...
	function updateGroupFamilyMember(){
		...
		//检查核心企业是否已是自身集团成员
 		var sIsGroupMember = RunJavaMethod("com.amarsoft.app.als.customer.group.action.GroupCustomerManage","isGroupCustomer1","memberCustomerID="+sKeyMemberCustomerID+",GroupID="+sGroupID);//String MgtOrgID,String KeyMemberCustomerID,String MgtUserID,String GroupType2,String oldGroupType2,String oldMgtOrgID,String oldMgtUserID,String oldKeyMemberCustomerID,String GroupID,String UserID,String IsGroupMember
		RunJavaMethodTrans("com.amarsoft.app.als.customer.group.action.UpdateGroupCustomerAction","updateGroupCustomerAction","mgtOrgID="+sMgtOrgID+",keyMemberCustomerID="+sKeyMemberCustomerID+",mgtUserID="+sMgtUserID+",groupType2="+sGroupType2+",oldGroupType2="+oldGroupType2+",oldMgtOrgID="+oldMgtOrgID+",oldMgtUserID="+oldMgtUserID+",oldKeyMemberCustomerID="+oldKeyMemberCustomerID+",groupID="+sGroupID+",userID="+sUserID+",isGroupMember="+sIsGroupMember);
