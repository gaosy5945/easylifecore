...
<html lang="en" xml:lang="en" xmlns="http://www.w3.org/1999/xhtml">
    ...
    <body>
        <table cellspacing="0" cellpadding="0" class="main">
            ...
            <tr>
                <td class="main-body">
                    <div id="bodyleft" class="main-body-left"><iframe id="myleft" src="<%=sWebRootPath%>/AppMain/Blank.html" name="myleft"></iframe></div>
                    <div id="bodyright" class="main-body-right"><iframe id="myright" src="<%=sWebRootPath%>/AppMain/Blank.html" name="myright"></iframe></div>
...
<script type="text/javascript" language="javascript">
	...
	function reloadTree(){		
		    ...
		    var sRightType = "<%=sRightType%>";	  
		 	if (typeof(sGroupID) == "undefined" || sGroupID.length == 0){
		 	       OpenPage("/AppMain/Blank.jsp?TextToShow=获取集团信息失败,集团客户编号为空!","myleft");
		 	       return;
		 	}
		    if(sKeyMemberCustomerID.length == 0){
		    	   OpenPage("/AppMain/Blank.jsp?TextToShow=母公司客户编号未填写！","myleft");
		           return;
		    }	
			...
			var sTreeViewDetail2="<%=sRefVersionSeq%>";
		   	if(<%=!Version.isVersion(sGroupID,sRefVersionSeq)%>)
	   			OpenPage("/AppMain/Blank.jsp?TextToShow=该集团客户的系统信息暂时还未建好，请稍后...","myright");
		   	else
		   		openIFram("sys","myright","TreeViewDetail1="+sTreeViewDetail1+"&TreeViewDetail2="+sTreeViewDetail2+"&RightType="+sRightType+"&FamilyMapStatus="+<%=sFamilyMapStatus%>+"&GroupID=<%=sGroupID%>&GroupName=<%=sGroupName%>&KeyMemberCustomerID=<%=sKeyMemberCustomerID%>&RefVersionSeq=<%=sRefVersionSeq%>&CurrentVersionSeq=<%=sCurrentVersionSeq%>&GroupType1=<%=sGroupType1%>");
	}
	//仅查看系统信息
  ...
  //对比查看自动搜索结果-系统信息
    function autoindexAndSystem(){
	    ...
	    if(!sInsertNodeFlag){
		    OpenPage("/CustomerManage/GroupManage/GroupRelationSearch.jsp?GroupID="+sGroupID+"&GroupName="+sGroupName+"&KeyMemberCustomerID="+sKeyMemberCustomerID+"&RightType="+sRightType,"myleft");
		}   
		//系统信息
		var sTreeViewDetail1="<%=sFamilyMapStatus%>";
		var sTreeViewDetail2="<%=sRefVersionSeq%>";
