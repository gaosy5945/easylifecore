...
<%	
	...
	String OTCSerialNo = CurPage.getParameter("OTCSerialNo");
	if(OTCSerialNo == null) OTCSerialNo = "";

	ASObjectModel doTemp = new ASObjectModel("MyOrgCustomerTagList");
	...
	String sButtons[][] = {
			...
			{"true","","Button","引入客户","引入客户","importCustomer()","","","","",""},
			{"true","","Button","删除客户","删除客户","del()","","","","btn_icon_delete",""},
...
<script type="text/javascript" src="<%=sWebRootPath%>/CustomerManage/js/CustomerManage.js"></script><script type="text/javascript">
	function alterTag(){
		OpenPage("/CustomerManage/MyOrgCustomerInfo.jsp?SerialNo="+"<%=serialNo%>","frameright","");
	}
	...
	function adjustTag(){
		...
		var result = CustomerManage.adjustOrgTag(recordArray, relaCustomerIDs, userID, relaSerialNos, inputDate, inputOrgID);
		if(result == "SUCCEED"){
			alert("分组调整成功！");
			reloadSelf();
		}else{
	...
	function importCustomer(){
         ...
         var returnValue = AsDialog.SelectGridValue('MyImportCustomer',"<%=CurUser.getUserID()%>,<%=CurUser.getOrgID()%>,03",'CUSTOMERID','',true,sStyle);
	...
	function reloadCustomer(){
		OpenPage("/CustomerManage/MyOrgCustomerTagList.jsp?SerialNo="+"<%=serialNo%>", "frameright"); 
	}
</script>
