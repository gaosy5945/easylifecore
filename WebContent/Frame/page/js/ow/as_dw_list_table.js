...
function TableBuilder(tableId,heads,datas,range,uiparams){
	...
	this.setSelfAdaptWidths(uiparams.checkFormat,uiparams.defaultWidths);
	/*
...
//自适应宽度调整
TableBuilder.prototype.setSelfAdaptWidths = function(checkFormat,defaultWidths){
	...
	if(defaultWidths){
		for(var i=0;i<defaultWidths.length;i++)
			if(defaultWidths[i])this.defaultWidths[i]=defaultWidths[i];
	}
...
TableBuilder.prototype.createTableHTML = function(){
	...
	var _flag_filter = window["aDWfilterTitles"] && window["aDWfilterTitles"].length > 0 && window["aDWfilterTitles"][0].length > 0;
	sbResult.append('<th><div class="list_gridCell_narrow');
	if(_flag_filter) sbResult.append(' list_gridCell_search" onclick="FilterAreaShow(this, event)"');
	else sbResult.append('"');
	sbResult.append('>&nbsp;</div></div></th>');
	//根据冻结的列数，输出标题
	...
	for(var i=0;i<tableDatas[this.tableId].length;i++){
		...
		var rownum= (i+1 + iCurPage*iPageSize);
		if(typeof(showRowNum)=="undefined"||showRowNum!="None"){
			
		}
		else{
			rownum="";
		}
		sbResult.append('<td class="list_all_no"  onDblClick=""><div class="list_gridCell_narrow" >'+ rownum+'</div></td>');
		...
		for(var j=0;j<this.lockCols;j++){
			...
			var sColInnerBtEvent = "";
			if(this.uiparams && this.uiparams.colInnerBtEvent)
				sColInnerBtEvent = this.uiparams.colInnerBtEvent[j];
			...
			if(this.uiparams && this.uiparams.colReadOnly && this.uiparams.colReadOnly[j]=="0"){
				...
				sValue= this.createInput("INPUT_"+ this.tableId +"_" + this.uiparams.colNames[j], this.uiparams.colEditStyle[j], sValue,sUnit, this.range[j], i, j,sCheckFormat,sColInnerBtEvent);
			}else{
	...
	sbResult.append('<div style=" width:100%;overflow-y:scroll; overflow-x:hidden" id="'+ this.tableId +'_float" class="list_topdiv_header"');
	//添加标题拖拉事件
	...
	for(var i=0;i<tableDatas[this.tableId].length;i++){
		...
		for(var j=this.lockCols;j<tableDatas[this.tableId][i].length;j++){
			...
			if(this.uiparams && this.uiparams.colReadOnly && this.uiparams.colReadOnly[j]=="0"){
				...
				sValue = TableBuilder.convertJS2HTML(tableDatas[this.tableId][i][j],sCheckFormat);
				sValue = '<span id="INPUT_'+ this.tableId + "_" + this.uiparams.colNames[j] + "_" + i+ "_" + j +'" class="list_event_width">'+ this.getTrueValue(j,sValue) + sUnit+ "</span>" ;
			}
			if(this.htmlStyle && this.htmlStyle[j])
...
TableBuilder.TRMouseOver = function(index,objleft,objright,visitedColor,evt){
	...
	if(objright){//选中右边的情况
		...
		if(index != TableBuilder.iCurrentRow[tableId])
			objright.style.backgroundColor = objright.getAttribute("lightColor");
		//获得冻结区域对应的行对象
		objLeft = document.getElementById("TR_Left_" + tableId+ "_" + index);
...
//创建input
TableBuilder.prototype.createInput = function(inputname,inputType,inputValue,unit,inputRange,row,col,checkFormat,sColInnerBtEvent){
	return TableBuilder.createInput(this.tableId,inputname,inputType,inputValue,unit,inputRange,row,col,checkFormat,sColInnerBtEvent);
};
...
TableBuilder.getCheckBoxValue=function(objname){
...
TableBuilder.isInArray = function(arr,str){
	if(!arr)return false;
	for(var i=0;i<arr.length;i++){
		if(arr[i]==str)
			return true;
...
TableBuilder.createInput = function(tableId,inputname,inputType,inputValue,unit,inputRange,row,col,checkFormat,sColInnerBtEvent){
	...
	if(inputType == 'select'){
		...
		for(var i=0;i<inputRange.length;i+=2){
			var sel = '';
			if(inputRange[i]=='')continue;
			if(TableBuilder.isInArray(inputValue.split(","),inputRange[i]))sel = 'checked';
			result.append('<input onfocus="clearErrors(this)" '+sHtmlStyle+' id="'+inputId+'_'+ i +'" name="'+inputname+'" type="'+ inputType +'" value="'+ inputRange[i] +'" '+sel+' onclick="tableDatas[\''+ tableId +'\']['+ row +']['+ col +']=TableBuilder.getCheckBoxValue(\''+inputname+'\')">' + inputRange[i+1]);
		}
		...
		var sSelectDate = "SelectDate(this,'yyyy/MM/dd','"+ sStartDate +"','"+ sEndDate +"')";
		var sInnerBtClass = "inner_bt_a";
		if(!sColInnerBtEvent){
			sColInnerBtEvent = "SelectDate(this.previousSibling,'yyyy/MM/dd','"+ sStartDate +"','"+ sEndDate +"')";
			sInnerBtClass += " inner_bt_date";
		}
		result.append('<span class="inner_bt_span">');
		result.append('<input onfocus="clearErrors(this)" onclick="'+sSelectDate+'" id="'+inputId+'" '+sHtmlStyle+' '+sLimit+' class="list_all_input_1" name="'+inputname+'" onblur="MRCheckDate(this);tableDatas[\''+ tableId +'\']['+ row +']['+ col +']=this.value" type="text" value="'+ inputValue +'" onchange="tableDatas[\''+ tableId +'\']['+ row +']['+ col +']=this.value">');
		result.append("<a onclick=\""+sColInnerBtEvent+";return false;\" href=\"javascript:void(0);\" class=\""+sInnerBtClass+"\" >&nbsp;</a>");
		result.append('</span>');
		...
		if(sColInnerBtEvent) result.append('<span class="inner_bt_span">');
		var readonly="";
		if(sColInnerBtEvent) readonly="readonly";
		result.append('<input onfocus="clearErrors(this)" '+readonly+'  id="'+inputId+'" '+sHtmlStyle+' '+sLimit+' class="list_all_input_1" name="'+inputname+'" type="text" value="'+ inputValue +'" onblur="tableDatas[\''+ tableId +'\']['+ row +']['+ col +']=this.value" onkeyup="tableDatas[\''+ tableId +'\']['+ row +']['+ col +']=this.value" onchange="'+sOnchange+'" '+sExtAttr+'>');
		if(sColInnerBtEvent) result.append("<a onclick=\""+sColInnerBtEvent+";return false;\" href=\"javascript:void(0);\" class=\"inner_bt_a\" >&nbsp;</a>");
		if(sColInnerBtEvent) result.append('</span>');
	}
