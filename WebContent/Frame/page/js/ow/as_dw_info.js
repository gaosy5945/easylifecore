...
function getErrorLabel(colName){
	var elements = $("label");
	for(var i=0;i<elements.length;i++){
		if($(elements[i]).attr("for")==colName)
			return elements[i];
	}
	return undefined;
}
...
function tipsTrMouseOver(obj, tips){
	if(!tips) tips = $(".info_tips:not(:empty)", obj);
	tips.show();
	$(obj).bind("mouseleave", mouseleave);
	function mouseleave(){
		tips.hide();
		$(obj).unbind("mouseleave", mouseleave);
	}
}
...
function setItemRequired(dwname,sColNames,value){
	if(value)
		showItemRequired(dwname,sColNames);
	else
		hideItemRequired(dwname,sColNames);
}
function showItemRequired(dwname,sColNames){
	//alert("sColName=" + sColName);
	var sColNameArray = sColNames.split(",");
	for(var i = 0; i < sColNameArray.length; i ++)
	{
		var sColName = sColNameArray[i];
		var sColIndex = getColIndexFromName(sColName);
		//alert(sColIndex);
		...
		if(oSpanInput){
			...
			if(!(/([<]SPAN style="COLOR:.*#ff0000"[>][<]DELETED\d*>.[<].DELETED\d*[>][<].SPAN[>])/gi.test(sHtml.toUpperCase()))){
				oSpanInput.innerHTML = sHtml.substring(0,sHtml.toUpperCase().lastIndexOf("</SPAN>")) + sSP+"</span>";
			}
		}else{
			var delspan = document.getElementById("req_"+sColIndex);
			if(delspan)
				delspan.style.display = 'inline';
			else{
				//$("#Title_" + sColIndex).after('<span style="color:#ff0000" id="req_'+sColIndex+'"><DELETED1>*</DELETED1></span>');
				$('<span style="color:#ff0000" id="req_'+sColIndex+'"><DELETED1>*</DELETED1></span>').appendTo($("#Title_" + sColIndex).next());
			}
		...
		try{
			if(document.getElementById(sColName))
				...
				$(document.getElementsByName(sColName)).rules("add",{required0 : true,messages:{required0:title}});
		}
		catch(e){
			...
			if(!rules[sColName].required0){
				...
				if(!messages[sColName]) messages[sColName] = new Object();
				messages[sColName].required0=title;
			}
		}
	}
	
	//alert($("#SERIALIZED_ASD").val());
	//更新校验
	$.ajax({
		   type: "POST",
		   ...
		   processData: false,
		   async:false,
		   data: "DataObject=" + $("#SERIALIZED_ASD").val()  + "&ColName="+ sColNames + "&Required=1",
...
function hideItemRequired(dwname,sColNames){
	//alert(sColName);
	var sColNameArray = sColNames.split(",");
	for(var i = 0; i < sColNameArray.length; i ++)
	{
		...
		if(oSpanInput){
			...
			sHtml = sHtml.replace(/([<]SPAN style="COLOR:.*#ff0000"[>][<]DELETED\d*>.[<].DELETED\d*[>][<].SPAN[>])/gi,"");
			//alert(sHtml);
	...
	$.ajax({
		   ...
		   processData: false,
		   async:false,
		   data: "DataObject=" + $("#SERIALIZED_ASD").val()  + "&ColName="+ sColNames + "&Required=0",
		   success: function(msg){
			  if(msg.substring(0,5)=="fail:"){
				  alert('校验更新失败:' +msg.substring(5));
				  return;
...
function setReadOnly(obj, value) {  
    if (obj) {  
    	if (obj.type == 'radio' || obj.type == 'checkbox') {  
    		...
    		//alert(flatSelect["input"].parent().html());
    		if(value){
...
function setGridValuePretreat(sDoNo, sArgs, sFieldValues, sSelected,manualQueryFlag){
	if(!sFieldValues) return;
	var regexp = /#{[A-Za-z0-9]+}/g;
	if(typeof sArgs == "string"){
		var cols = sArgs.match(regexp);
		if(cols) for(var i = 0; i < cols.length; i++) {
			var sCol = cols[i].substring(2,cols[i].length-1);
			var sColValue = getItemValue(0, 0, sCol);
			sArgs = sArgs.replace("#{"+sCol+"}", sColValue);
		}
	}
	if(typeof sSelected == "string"){
		var cols = sSelected.match(regexp);
		if(cols) for(var i = 0; i < cols.length; i++) {
			var sCol = cols[i].substring(2,cols[i].length-1);
			var sColValue = getItemValue(0, 0, sCol);
			sSelected = sSelected.replace("#{"+sCol+"}", sColValue);
		}
	}
	//alert("sDoNo="+sDoNo+" sArgs="+sArgs+" sFieldValues="+sFieldValues+" sSelected="+sSelected);
	return AsDialog.SetGridValue(sDoNo, sArgs, sFieldValues, sSelected,'',manualQueryFlag);
}