...
<%
	  ...
	  for(int i=0;i<list.size();i++){
		  ...
		  for(int j=0;j<fieldArray.length;j++){
			  sTmpValue = obj.getAttribute(fieldArray[j]).getString();
			  if(sTmpValue == null || "".equals(sTmpValue)){
				  sValue = "&nbsp;";
			  }else{
				  if(PublicFuns.strInArr(aYesNoFields,fieldArray[j])>-1){//0 1 处理
					  sValue = "1".equals(sTmpValue)?"是":"否";
				  }else if(fieldArray[j].equals("COLCHECKFORMAT")){
					  sValue = CodeManager.getItemName("CheckFormat",sTmpValue);
				  }else if(fieldArray[j].equals("COLALIGN")){
					  sValue = CodeManager.getItemName("ColAlign",sTmpValue);
					  ...
					  sValue = PublicFuns.filterHtmlStr(sTmpValue);
				  }
				  if(sValue == null || "".equals(sValue))sValue = "&nbsp;";
		  	  }
