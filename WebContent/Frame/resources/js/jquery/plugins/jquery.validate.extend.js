		...
		if(params[i] && params[i]!=''){
			...
			if(sParamName.length > 7 && sParamName.substring(0,7) == '$Const:' && sParamName.indexOf("=")>-1){
				...
				if(sParamName.indexOf(".")>0){
					var p=sParamName.split(".");
					dwname=ALSObjectWindowFunctions.getSubObjectWindowName(0,p[0]);
					sParamName=p[1];
				}
				sParamValue = getItemValue(dwname,index,sParamName.toUpperCase());
				if(sParamValue==undefined)
					sParamValue=getItemValue(dwname,index,sParamName);
				//alert(sParamValue);
			}
...
function errorPlaceRule(error, element) {
	...
	if (element.is(':radio') || element.is(':checkbox')) {
	  ...
	  else error.insertAfter(tlabel?tlabel:element);
