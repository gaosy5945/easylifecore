...
<%
	...
	if(sOldObjectType.equals("FileManageView") && sObjectType.equals("Image")){
		...
		//查找对应的docId, 放进list
		resultSet= Sqlca.getResultSet(
				"select  documentId  from  ECM_PAGE where objectType='"+sObjectType+"' and objectNo='"+sObjectNo+"' "
				+" and documentId is not null order by serialno"
		);
	}
	...
	for(int i=0; i<list.size(); i++){
		...
		"<div class='info' style='display: none;'>"+(content==null?"":content.getDesc())+"</div>"+
				"<div class='content' id='"+i+"'><img name='"+i+"' style='width:206px ' src='"+sNewPath +"/servlet/ContentServlet?Id="+list.get(i)+"' ></img></div>"+ 
				"</div>";
	}
...
$(document).ready(function(){
	...
	document.onkeydown=function(){
		...
		if(key == 37){
			n--;
			var div = document.getElementById('idContainer').childNodes;//获取id为imglist下所有子元素；
			var img1 = div[0];
			var imSrc = img1.src;
	...
	function getDivimg(id){
		var div = document.getElementById('thumbnail_img').childNodes;//获取id为imglist下所有子元素；
		var name=div[id].outerHTML;
		var div1 = document.getElementById(id).childNodes;
		var img=div1[0];
		return img.src
	}
});
