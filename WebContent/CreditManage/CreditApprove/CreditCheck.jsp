							...
							<tbody>
								<%
									for (int j = 0; j < checkItemList.size(); j++) {
												...
												//获取对应checkitem的操作指引
												Map<String,String> operinstructs = CreditCheckConfig.getCheckItemOperInstructs(checkItem, Sqlca, checkItemDatas);
								...
								%>
								<tr>
									<td rowspan="2" class="list_all_td" align="center"  vAlign="middle"><%=count%>.</td>
									<td rowspan="2" class="list_all_td" align="center"  vAlign="middle"><%=checkItemLabel%></td>
									<td colspan="3" class="list_all_td" align="left" vAlign="middle">
										<div><font style="color: dimgrey; font-style: italic;" >操作指引:</font></div>
											<%
											if (operinstructs == null || operinstructs.size() == 0) {
											%> 
												 <span><font style="color: blue; font-style: italic;">该核查项无操作指引</font></span>
											<%
											}
											int instruno = 1;
											for (String key : operinstructs.keySet()) {
											%> 
												<div ><font color="dimgrey"><%=instruno++%>.<%=operinstructs.get(key)%></font></div>
											<%
											}
											%>							
									</td>
