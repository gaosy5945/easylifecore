...
<%
	...
	String Status = CurPage.getParameter("Status");if(Status == null)Status = "";
	//抵质押变更传入参数
	...
	inputParameter.setAttributeValue("BusinessObjectType", objType);
	inputParameter.setAttributeValue("BusinessObjectNo", objNo);
	
	ASObjectWindow dwTemp = ObjectWindowHelper.createObjectWindow_Info("GuarantyAssetView",inputParameter, CurPage, request);
	ASObjectModel doTemp = (ASObjectModel)dwTemp.getDataObject();
	
	//判断押品是否可修改，对于引用的押品，若存在已生效的担保合同（除该笔），核心要素不可修改
	String Attribute4 = "";
	if(!assetSerialNo.equals("")){
		...
		if("false".equals(editFlag)){
			com.amarsoft.dict.als.object.Item temp = com.amarsoft.dict.als.cache.CodeCache.getItem("AssetType", assetType);
			if(temp != null){
				Attribute4 = temp.getAttribute4();
			}
		}
	}
	...
	String sButtons[][] = {
			...
			{"false","","Button","办妥预抵押","办妥预抵押","changeStatus1()","","","",""},
			{"false","","Button","办妥正式抵押","办妥正式抵押","changeStatus2()","","","",""}
		};
	//如果是一类资料出入库管理中查看押品详情不可修改数据
	if("DocType".equals(docFlag) || "".equals(mode)){
		 sButtons[0][0] = "false";
	}
	if("".equals(mode)){
		 sButtons[1][0] = "true";
...
<script type="text/javascript">
		...
		if(result[0] == "01"){//通过
			...
			else
			{
				...
				if(assetSerialNo == "false"){//本地没有该押品，在本地新建押品数据，接口返回值更新至本地
					...
					if(newAssetNo){
						var sReturn = AsControl.PopComp("/CreditManage/CreditApply/ImportCollateralInfo.jsp", "SerialNo=&GCSerialNo="+gcSerialNo+"&ObjectNo=<%=objectNo%>&ObjectType=<%=objectType%>&TemplateNo="+templateNo[1]+"&AssetType="+assetType+"&AssetSerialNo="+newAssetNo, "");
						self.close();
					}
					else{
						alert("引入失败！");
	...
	function refreshCurrentPage(){
		...
		AsControl.OpenView("/CreditManage/CreditApply/GuarantyCollateralInfo.jsp", "SerialNo="+serialNo+"&AssetSerialNo="+assetSerialNo+
	...
	function init(){
		...
		//存在已生效的担保合同（除该笔），将该押品的核心要素置为不可修改
		var Attribute4 = "<%=Attribute4%>".split(",");
		for(var i = 0;i < Attribute4.length;i++){ 
			setItemDisabled(subdwname, 0, Attribute4[i], true);
		}
	}
