...
<%/*~BEGIN~可编辑区~[Editable=true;CodeAreaID=List04;Describe=定义按钮;]~*/%>
	...
	String sButtons[][] = {
			{(sOccurType.equals("020")?"true":"false"),"","Button","添加关联借据","关联一笔已有的业务","newRecord()","","","",""},
			{"true","","Button","借据详情","查看借据详情","view()","","","",""},
			{"true","","Button","相关合同详情","查看相关合同详情","viewTab()","","","",""},
			{(sOccurType.equals("020")?"true":"false"),"","Button","删除","删除关联","deleteRecord()","","","",""},
		};
	%>
<%/*~END~*/%>
...
<%/*~BEGIN~可编辑区~[Editable=false;CodeAreaID=List06;Describe=自定义函数;]~*/%>
	...
	{		
		...
		if(typeof(sRelativeSerialNo) != "undefined" && sRelativeSerialNo != "") 
		{
			...
			//按照借据关联	
			sReturn = RunJavaMethodTrans("com.amarsoft.app.lending.bizlets.GetColValue","getColValue","colName=ObjectNo,tableName="+"<%=sRTableName%>"+",whereClause="+"String@SerialNo@"+"<%=sObjectNo%>"+"@String@ObjectType@BusinessDueBill@String@ObjectNo@"+sRelativeSerialNo);
			...
			//新增业务与所选合同的关联关系
			//sReturn = RunJavaMethodTrans("com.amarsoft.app.bizmethod.BusinessManage","insertRelative","paras=serialNo@@"+"<%=sObjectNo%>"+"@~@ObjectType@@BusinessContract@~@objectNo@@"+sRelativeSerialNo+"@~@tableName@@"+"<%=sRTableName%>");
			//新增业务与所选借据的关联关系
			sReturn = RunJavaMethodTrans("com.amarsoft.app.bizmethod.BusinessManage","insertRelative","paras=serialNo@@"+"<%=sObjectNo%>"+"@~@ObjectType@@BusinessDueBill@~@objectNo@@"+sRelativeSerialNo+"@~@tableName@@"+"<%=sRTableName%>");
