...
<%/*~BEGIN~可编辑区~[Editable=true;CodeAreaID=List04;Describe=定义按钮;]~*/%>
	...
	//依次为：
		...
		//6.资源图片路径
	String sButtons[][] = {
		{"true","","Button","最终审批意见详情","最终审批意见详情","viewTab()","","","",""},
		{"true","","Button","登记合同","登记合同","BookInContract()","","","",""}
		}; 
...
<%/*~BEGIN~可编辑区~[Editable=false;CodeAreaID=List06;Describe=自定义函数;]~*/%>
	...
	function BookInContract(){
		...
		var sApplyType = getItemValue(0,getRow(),"ApplyType");//申请类型
		var sObjectType = "ApproveApply";//对象类型
		...
		if (typeof(sSerialNo)=="undefined" || sSerialNo.length==0){
		    ...
		    if(sApplyType=="CreditLineApply"||sApplyType=="DependentApply"){
				 alert("根据最终审批意见生成合同成功，合同流水号["+sReturn+"]！\n\r请继续填写合同要素并“保存”或稍后在“待登记完成的授信额度”列表中选择该合同并填写合同要素！");
            }else{
