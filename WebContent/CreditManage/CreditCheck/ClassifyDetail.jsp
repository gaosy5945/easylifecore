...
<%@include file="/IncludeBegin.jsp"%><%
	...
	//定义变量：动作、项目名称、项目值、代码、值方法、值类型、Sql语句
	...
	//查询进行风险分类的业务品种、客户名称和余额
	if(sResultType.equals("BusinessContract")){ //按合同
		sObjectTable = "BUSINESS_CONTRACT";
	}else if(sResultType.equals("BusinessDueBill")){ //按借据
...
%>
		<script type="text/javascript"> 
			alert(getMessageText("ALS70666"));//该期资产风险分类信息删除完成！
			self.close();
		</script> 
<%		
	}
...
<script type="text/javascript"> 
	/*~[Describe=测算;InputParam=无;OutPutParam=SerialNo;]~*/
  	...
  	/*~[Describe=保存;InputParam=无;OutPutParam=SerialNo;]~*/
   	...
   	function deleteData(){
		if (confirm(getMessageText("ALS70667"))){ //将要删除该该期资产风险分类信息，继续吗？
	        var sParameter = "1=1"+
		        "&OpenType= "+
		        "&Action=_DELETE_"+
		        //"&ClassifyType="+sClassifyType+
	...
	function ClassifyChange(){
		...
		if(document.report.R2.value == "1"){
			...
			if((document.report.R3.value=="0")&&(document.report.R4.value==""))
				alert(getMessageText("ALS70663"));//请选择直接认定为损失类的具体原因或输入其他原因！
		}else{
			...
			if(document.report.R5.value == "1"){
				...
				if((document.report.R6.value=="0")&&(document.report.R7.value==""))
					alert(getMessageText("ALS70664"));//请选择直接认定为可疑类的具体原因或输入其他原因！
			}else{
				document.report.R3.disabled=true;	
				document.report.R4.disabled=true;
				document.report.R5.disabled=false;
  ...
  <tr > 
    <td height="1"> 
      <table width="80%" border="0" cellspace="0" cellpad="2" align="center">
        ...
        %>
          <td align="center">
          	<%=new Button("保存", "保存", "updateData()", "", "").getHtmlText()%>
    	  </td>
    	  <td align="left">
    	  	<%=new Button("测算", "测算", "classify()", "", "").getHtmlText()%>
    	  </td>
          <!-- <td ><%=new Button("删除", "删除", "deleteData()", "", "").getHtmlText()%></td>-->
         <%
         	}
         %>
        </tr>
...
%>
		<script type="text/javascript">
			...
			if(sMessage == "FinishSave")
				alert(getMessageText("ALS70668"));//该期资产风险分类数据保存完成！
			if(sMessage == "FinishCalculate")
				alert(getMessageText("ALS70669"));//该期资产风险分类测算完成！
			if(sMessage == "UnfinishCalculate"){
				alert(getMessageText("ALS70670"));//该期资产风险分类测算没有正常完成！
				alert("<%=sException%>");
			}
		</script>
<%	 
