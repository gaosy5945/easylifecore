...
<%@page import="com.amarsoft.biz.process.action.*" %><% 
	/*
    ...
    try{
		...
		if(sType.equals("Batch")){
            ...
            if(iTCount > 0){
	            ...
	            Sqlca.executeSQL(sSql);
	            //初始化五级分类流程
	            BusinessProcessAction bpAction = new BusinessProcessAction(Sqlca);
	            bpAction.setApplyType("ClassifyApplyX");    	//申请类型
	            bpAction.setObjectType(sObjectType);    		//对象类型
	            bpAction.setObjectNo(sSerialNo);        		//对象编号
	            bpAction.setUserID(CurUser.getUserID());    	//操作员编号
	            bpAction.setProcessDefID("ClassifyFlow");   	//流程编号
	            bpAction.start();
            }
        }
