<%@ page contentType="text/html; charset=GBK"%><%@
 include file="/IncludeBegin.jsp"%><%
	...
	String sButtons[][] = {	
			{(sSignalStatus.equals("20")?"true":"false"),"","Button","签署意见","填写该笔预警信息的认定意见","newOpinion()","","","",""},		
			{"true","","Button","查看意见","查看/修改认定意见详情","viewOpinion()","","","",""},
			{"true","","Button","预警详情","查看/修改详情","viewAndEdit()","","","",""},
			{(sSignalStatus.equals("20")?"true":"false"),"","Button","填写预警检查报告","填写预警检查报告","writeReport()","","","",""},
			{(sSignalStatus.equals("20")?"true":"false"),"","Button","提交","提交所选中的记录","commitRecord()","","","",""},
			{(sSignalStatus.equals("30")?"true":"false"),"","Button","查看预警检查报告","查看预警检查报告","viewReport()","","","",""},		
	};
...
<script type="text/javascript">
	...
	function viewReport(){
		...
		sReturn=RunJavaMethodTrans("com.amarsoft.app.lending.bizlets.GetColValue","getColValue","colName=SerialNo,tableName=INSPECT_INFO,whereClause=String@ObjectType@RiskSignal@String@ObjectNo@"+sSerialNo);
	...
	function commitRecord(){
		...
		//检查是否签署意见
		sReturn=RunJavaMethodTrans("com.amarsoft.app.lending.bizlets.GetColValue","getColValue","colName=Opinion,tableName=RISKSIGNAL_OPINION,whereClause=String@ObjectNo@"+sSerialNo);
		...
		if(confirm(getHtmlMessage('17'))){ //确信需要提交该记录吗？
			...
			if(typeof(sSignalStatus) != "undefined" && sSignalStatus.length != 0 && sSignalStatus != '_none_'){
				//提交操作
				sReturn = RunJavaMethodTrans("com.amarsoft.app.lending.bizlets.UpdateColValue","updateColValue","colName=String@SignalStatus@"+sSignalStatus+",tableName=RISK_SIGNAL,whereClause=String@SerialNo@"+sSerialNo);
				if(typeof(sReturn) == "undefined" || sReturn.length == 0) {					
