...
/*~[Describe=系统自动决策;InputParam=无;OutPutParam=无;]~*/
	...
	function invokeRE(sobjectNo,sobjectType){
		...
		if(typeof(sRuleID)=="undefined"||sRuleID.length==0){
 		...
 		var returnvalue = AsControl.RunJavaMethodTrans("com.amarsoft.app.rule.RuleInvoke","run","sceneID="+"CFRule"+",ruleID="+"CF010"+",ruleType="+"RuleFlow"+",callType="+<%=sCallType%>+",objectNo="+sobjectNo+",objectType="+sobjectType);
		...
		<div>
		  <table class="info_data_tablecommon info_group_table" width='100%'>
		  	<thead class='list_topdiv_header'>
		      	<th style="background:transparent;border:0px" >产品定价结果</th>
		    </thead>
		  </table>
		  <table class="info_data_tablecommon info_group_table" width='100%'>
		  	 <thead class='list_topdiv_header'>
		    ...
		    int i=0;
		    List<String[]>  showItem = rrp.sortPrice(map);
		    String resultItem = null;
			String resultValue =null;
			String groupname =null;
		    for(int k=0;i<showItem.size();k++){
		    	String[] temp = showItem.get(i);
		    	groupname= temp[0];
		    	resultItem = temp[1];
		    	resultValue = temp[2]; 
		    %>	    
