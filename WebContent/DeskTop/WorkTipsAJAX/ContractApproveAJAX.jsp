...
<% 	
	...
	if(sFlag.equals("0")){
		...
		sSql= 	" select getBusinessName(BA.BusinessType)||'&nbsp;['||BA.CustomerName||']'||'&nbsp;', "+
				" BA.BusinessSum,BA.OccurDate";
		sSql = sSql+ WhereCase;
		rsTips = Sqlca.getResultSet(new SqlObject(sSql).setParameter("OperateUserID",CurUser.getUserID()));
		while(rsTips.next()){
			if (rsTips.getString(3).substring(0,10).equals(StringFunction.getToday()))//判断是否是当天提交的申请，如果是则不显示提示灯
				sTipsFlag="&nbsp;&nbsp;&nbsp;";
			else
			sTipsFlag="<img src='"+sWebRootPath+"/DeskTop/WorkTipsAJAX/icon_alert.gif' width=12 height=12 alt=''>&nbsp;";
%>				<tr>
