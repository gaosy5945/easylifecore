...
<body class="pagebackground" leftmargin="0" topmargin="0" id="mybody" style="background:transparent;">
	<div id="WindowDiv">
		<div id="WorkPlanDiv">
	        ...
	        String sAllItemNo = "";
	    	//取出代码对象
	    	Item[] codeDef = CodeManager.getItems("ClewMain");
...
<script type="text/javascript">
	...
	function CountPlantformRecur(allItemNo, index)
	{
		...
		url=url+"?CompClientID=<%=SpecialTools.amarsoft2Real(sCompClientID)%>&ItemNo="+ItemNo+"&Type=0&CodeNo=ClewMain";
		xmlHttp.onreadystatechange = function(){
			...
			if (xmlHttp.readyState == 4  && xmlHttp.status == 200) {
				...
				//if(rs == "done") parent.clickWarning();
			}
			else{
	...
	function totalNum(message){
		 ...
		 //parent.window.setNum("PromptxNum",num); 
	}
	...
	function touchPlantform(ItemNo){
		...
		if(eid("DataList"+ItemNo).innerHTML == ""){
			var url="<%=sWebRootPath%>/DeskTop/WorkTipsAJAX/InvokerAjax.jsp";
			url=url+"?CompClientID=<%=SpecialTools.amarsoft2Real(sCompClientID)%>&ItemNo="+ItemNo+"&Type=1&CodeNo=ClewMain";
			xmlHttp.onreadystatechange = function(){
				var message = "";
