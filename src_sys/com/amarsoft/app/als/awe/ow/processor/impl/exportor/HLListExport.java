...
public class HLListExport extends ListExportServlet2 {
	public void dosubmit(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		try {
			...
			if (querier.getTotalCount()<= GPageSize) {//单页
				try {
					directExport(querier,this.asDataObject.getDONO() + "."+ this.exportFileType, req, resp);
	...
	protected void fillHeadAndData() throws Exception {
		...
		if (this.listJBO != null) {
			...
			for (int i = 0; i < this.listJBO.size(); i++) {
				...
				for (int j = 0; j < visibleColumnNames.length; j++) {
					...
					if (value != null) {
						if(visibleColumnNames[j].equals("DUEBILLNO")) 
							HLList.add(value);
						int iDot = 0;
		...
		//对于已导出的数据更新标记ExportStatus为1：已导出
		String sql = "update CreditHL_Info set ExportStatus='1' where duebillNo= ?";
		...
		for(int i=0;i<HLList.size();i++){
			ps.setString(1,(String)HLList.get(i));
			ps.addBatch(); 					
		}
		int insertedRows =  ps.executeBatch().length;
		if(insertedRows > 0){
			connection.commit();
		}else{
			connection.rollback();
