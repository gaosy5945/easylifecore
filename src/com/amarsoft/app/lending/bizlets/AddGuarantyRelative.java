...
public class AddGuarantyRelative extends Bizlet {
	...
	public Object run(Transaction Sqlca) throws Exception{
		...
		//GuarantyCheckAction gca = null;// new GuarantyCheckAction();//押品登记
		if(IsExist)	{	
			rs = Sqlca.getASResultSet(so);
			while (rs.next())
			{
				...
				if(iCount < 1)
				{
					...
					//押品与合同关联后需要创建押品登记记录 add by wmzhu 2014-05-23
					//gca.newGuarantyCheck(Sqlca, sGuarantyID, orgID, userID);
				}
				else{    //added by yzheng 2013-07-11
					String relationStatus = "";
					sSql = 	" select RelationStatus from GUARANTY_RELATIVE where ContractNo=:ContractNo and GuarantyID=:GuarantyID and ObjectNo=:ObjectNo";
					so = new SqlObject(sSql).setParameter("ContractNo", sContractNo).setParameter("GuarantyID", sGuarantyID).setParameter("ObjectNo", sSerialNo);
					rs1 = Sqlca.getASResultSet(so);
					if (rs1.next())
						relationStatus = rs1.getString("RelationStatus");
					rs1.getStatement().close();
					
					if(relationStatus.equals("010")){  //重复引入判断
						return "2";  
					}
					else{  //之前引入后又被删除
						sSql = 	" update GUARANTY_RELATIVE set RelationStatus = '010' where ContractNo=:ContractNo and GuarantyID=:GuarantyID and ObjectNo=:ObjectNo";
						so = new SqlObject(sSql).setParameter("ContractNo", sContractNo).setParameter("GuarantyID", sGuarantyID).setParameter("ObjectNo", sSerialNo);
						Sqlca.executeSQL(so);
