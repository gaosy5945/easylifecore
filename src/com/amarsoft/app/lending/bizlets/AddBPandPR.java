 ...
 * 				SerialNo：出账流水号
 * 		HistoryLog:
 * 				lyin 2014-01-23  如业务品种为银行承兑汇票贴现、商业承兑汇票贴现，将拷贝票据信息至BILL_INFO表中，用于建立票据信息和出账信息的关联
 */
...
public class AddBPandPR extends Bizlet {
	...
	public Object run(Transaction Sqlca) throws Exception{
		...
		*/
		...
		//拷贝合同阶段票据信息至出账阶段
		String sBillInfoSerialNo="";
		String newObjectType="PutOutApply";
		String sFieldValue="";
		int iFieldType;
		sSql =  " select * from BILL_INFO where ObjectType ='BusinessContract' and ObjectNo =:sObjectNo ";
		ASResultSet rs = Sqlca.getASResultSet(new SqlObject(sSql).setParameter("sObjectNo", sObjectNo));
		int iColumnCount = rs.getColumnCount();
		while(rs.next()){
			//获得票据信息流水号
			sBillInfoSerialNo = DBKeyHelp.getSerialNo("BILL_INFO","SerialNo",Sqlca);
			//插入票据信息
			sSql = " insert into BILL_INFO values('"+newObjectType+"','"+sSerialNo+"','"+sBillInfoSerialNo+"'";
