...
public class DefaultScenarioContextLoader extends ScenarioContextLoader{
	...
	private void loadScenarioModel() throws Exception{
		...
		BizObject bo = JBOFactory.getFactory().getManager("jbo.scenario.SCENARIO_CATALOG").createQuery("ScenarioID=:ScenarioID").setParameter("ScenarioID",scenarioID).getSingleResult(false);
		if(bo != null){
	...
	private void loadGroup() throws JBOException{
		...
		List<BizObject> jList = JBOFactory.getFactory().getManager("jbo.scenario.SCENARIO_GROUP")
				.createQuery(query)
	...
	private void loadItem(ItemGroup group) throws JBOException{
		...
		String query = "Select \"O.*\" from O,jbo.scenario.SCENARIO_RELATIVE SR where O.ScenarioID = SR.ScenarioID and O.ModelID = SR.ModelID "+
		...
		BizObjectQuery q =  JBOFactory.getFactory().getManager("jbo.scenario.SCENARIO_MODEL").createQuery(query);
	...
	private void loadSQLParameter() throws Exception{
		//查询参数SQL
		@SuppressWarnings("unchecked")
		List<BizObject> jList = JBOFactory.getFactory().getManager("jbo.scenario.SCENARIO_ARGS")
		...
		for(int i = 0; i < jList.size(); i++){
			BizObject bo = jList.get(i);
			String  ArgsString=SpecialTools.amarsoft2Real(bo.getAttribute("ArgsString").getString());
			String  QuerySQL=SpecialTools.amarsoft2Real(bo.getAttribute("QuerySQL").getString());
			list.add(new String[]{ArgsString,	// key
					QuerySQL});	// value SQL
		}
		//执行参数SQL
		for(int i=0;i<list.size();i++){
			...
			String argSql = AmarScriptTool.pretreat(context.getParameter(),arg[1]);
			//argSql=SpecialTools.amarsoft2Real(argSql);
			ARE.getLog().trace("场景业务参数SQL:"+argSql);
			ASResultSet rs = null;
			try{
				rs  = sqlca.getASResultSet(new SqlObject(argSql));	//执行参数配置的SQL
				if(rs.next()){
