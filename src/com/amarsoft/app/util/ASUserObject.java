...
public class ASUserObject implements Serializable{
	...
	static public ASUserObject getUser(String sUserID) throws Exception {
		...
		try {
			BizObjectQuery q = JBOFactory.getFactory().getManager(SystemConst.JAVA_USER_INFO).createQuery("UserID = :UserID and Status = :Status");
			...
			if (bo != null) {
				...
				user.setLoginID(bo.getAttribute("LoginID").getString());
			} else {
	...
	public boolean hasRole(String sRoleID) throws Exception {
		BizObjectManager bom = JBOFactory.getFactory().getManager(SystemConst.JAVA_USER_ROLE) ;
