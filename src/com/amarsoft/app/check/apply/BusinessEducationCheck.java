...
public class BusinessEducationCheck extends AlarmBiz {
	...
	public Object run(Transaction Sqlca) throws Exception {
		...
		else
		{
			...
			{
				boolean flag = false;
				ASResultSet rs = Sqlca.getResultSet(new SqlObject("select * from BUSINESS_EDUCATION where ObjectType = 'jbo.app.BUSINESS_APPLY' "
						+ "and ObjectNo = :ObjectNo").setParameter("ObjectNo", ba.getString("SerialNo")));
				if(rs.next()){
					flag = true;
				}
				if(!flag){
					putMsg("申请【"+ba.getString("CustomerName")+"】未录入留学/助学贷款信息！");
				}
				rs.close();
			}
		}
