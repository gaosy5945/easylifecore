...
public class ObjectModelCatListAction {
	private String DONO;
	...
	public String quickCopyCatalog() throws JBOException{
		...
		BizObject catalog = m.createQuery("DONO = :DONO").setParameter("DONO", DONO).getSingleResult(false);
		if (catalog == null)
		...
		List<BizObject> items = m.createQuery("DONO = :DONO").setParameter("DONO", DONO).getResultList(false);
		tx.join(m);
		for (BizObject bizObject : items) {
