 ...
 * 购房贷款附属信息
 */
public class BusinessHouseProcess extends ALSBusinessProcess implements BusinessObjectOWDeleter,BusinessObjectOWUpdater{
	...
	public List<BusinessObject> update(BusinessObject businessTrade, ALSBusinessProcess businessProcess) throws Exception {
		...
		BusinessObject assetRealty = businessTrade.getBusinessObject("jbo.guaranty.ASSET_REALTY");
		...
		if(!existFlag && "1".equals(isCollateral)){//此资产未被引为担保且需要引入为押品时才新建
			...
			assetInfo.setAttributeValue("InputOrgID",orgID);
			assetInfo.setAttributeValue("InputUserID",userID);
			...
			gc.setAttributeValue("GuarantyType", "02010");//房地产抵押
			...
			gr.setAttributeValue("Status", "0100");
	...
	public int delete(BusinessObject businessTrade, ALSBusinessProcess businessProcess) throws Exception {
		BusinessObject assetInfo = businessTrade.getBusinessObject("jbo.app.ASSET_INFO");
		BusinessObject assetRealty = businessTrade.getBusinessObject("jbo.guaranty.ASSET_REALTY");
		this.bomanager.deleteBusinessObject(assetRealty);
