...
public class ContractGuarantyChangeProcess extends ALSBusinessProcess
implements BusinessObjectOWDeleter,BusinessObjectOWUpdater{
	...
	public List<BusinessObject> update(BusinessObject guarantyContract,
		...
		if("020".equals(contractType)){
			String selectCLSql = " objectno=:OBjectNo and objectType=:ObjectType ";
			List<BusinessObject> clList = bomanager.loadBusinessObjects("jbo.cl.CL_INFO", selectCLSql, "ObjectNo", guarantyContract.getString("SerialNo"),"ObjectType", "jbo.guaranty.GUARANTY_CONTRACT");
			if(clList==null){
				...
				cl.setAttributeValue("ObjectNo", guarantyContract.getString("SerialNo"));
		...
		String arSerialNo = guarantyContract.getString("ARSerialNo");
		BusinessObject crTmp = bomanager.keyLoadBusinessObject("jbo.app.CONTRACT_RELATIVE_CHANGE", arSerialNo);
		if(crTmp!=null){
			crTmp.setAttributeValue("RelativeAmount", guarantyContract.getString("RelativeAmount"));
			this.bomanager.updateBusinessObject(crTmp);
		}else{
			BusinessObject relative = BusinessObject.createBusinessObject("jbo.app.CONTRACT_RELATIVE_CHANGE");
			relative.setAttributeValue("ObjectType","jbo.guaranty.GUARANTY_CONTRACT_CHANGE");
			relative.setAttributeValue("ObjectNo",guarantyContract.getString("SerialNo"));
			relative.setAttributeValue("ChangeFlag",guarantyContract.getString("ChangeFlag"));
			relative.setAttributeValue("ContractSerialNo",guarantyContract.getString("ContractSerialNo"));
			relative.setAttributeValue("relativeTransSerialno",guarantyContract.getString("relativeTransSerialno"));
			relative.setAttributeValue("relativeType",guarantyContract.getString("relativeType"));
			relative.setAttributeValue("ContractStatus",guarantyContract.getString("ContractStatus"));
			relative.setAttributeValue("Currency",guarantyContract.getString("Currency"));
			relative.setAttributeValue("RelativeAmount", guarantyContract.getString("RelativeAmount"));
			if("010".equals(contractType)){
				relative.setAttributeValue("RelativePercent", 1.0);   //100%
			}
			String applyRelativeNo=relative.getKeyString();
			if(StringX.isEmpty(applyRelativeNo)){
				relative.generateKey();
				applyRelativeNo = relative.getKeyString();
			}
			this.bomanager.updateBusinessObject(relative);
		}
