 ...
 *
 */
public class BankStaffImport extends AbstractExcelImport {
	...
	public boolean process(Map<String, DataElement> excelMap) {
		...
		try {
		    ...
		    String[] billString=new String[]{"CUSTOMERNAME","CERTID","CERTTYPE","GENDER",
		    		                                                     "MOBILETEL","EMAIL","REMARK"
		                                                                  };

		    String[] billStringName=new String[]{"客户名称","证件号","证件类型","性别",
		    																"手机号码","电子邮件","备注"};
			...
			String sex = excelMap.get("GENDER").getString();
			String gender = CodeGenerater.getItemNoByName(sex, "Sex");
			String mobileTel = excelMap.get("MOBILETEL").getString();
			String email = excelMap.get("EMAIL").getString();
			...
			if(flag){
				...
				bo.setAttributeValue("GENDER", gender);
				bo.setAttributeValue("MOBILETEL", mobileTel);
				bo.setAttributeValue("EMAIL", email);
