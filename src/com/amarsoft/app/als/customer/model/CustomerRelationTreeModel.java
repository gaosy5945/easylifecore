...
private String customerID;
	...
	private void setRelationTree(String sCustomerID, String sCustomerName, String parentSortNo, HTMLTreeView tviTemp, DecimalFormat format,String parent,List<String> searchedCustomer, int leaf, Transaction Sqlca) throws Exception {
		...
		//取客户所有关联关系
		BizObjectManager bm = JBOFactory.getFactory().getManager("jbo.customer.CUSTOMER_RELATIVE");
		List relaList = bm.createQuery("select distinct RelationShip from o where CustomerID=:CustomerID and exists(select 1 from jbo.customer.CUSTOMER_INFO CI where CI.CustomerID=o.RelativeCustomerID) order by RelationShip")
		...
		for(int i = 0; i < relaList.size(); i++){
			...
			List subList = bm.createQuery("select RelativeCustomerID from o where CustomerID=:CustomerID and RelationShip=:RelationShip and exists(select 1 from jbo.customer.CUSTOMER_INFO CI where CI.CustomerID=o.RelativeCustomerID) ")
			...
			for(int j = 0; j < subList.size() ; j++){
				...
				sRelativeID = relaCustbo.getAttribute("RelativeCustomerID").getString(); 
