...
public class CR_060 extends FormatDocData implements Serializable {
	...
	public boolean initObjectForRead() {
		ARE.getLog().trace("CR_060.initObject()");
			...
			if(sCustomerID!=null&& !"".equals(sCustomerID)){
				...
				if(providers.size()<=5){
					...
					for(int i=0;i<providers.size();i++){
						...
						extobj2[i].setAttr5(DataConvert.toMoney(provider.getAttribute("InvestmentSum").getDouble()/10000));
						extobj2[i].setAttr6(provider.getAttribute("WHETHEN1").getString());
					}
				}else if(providers.size()>5){
					extobj2 = new DocExtClass[5];
					for(int i=0;i<5;i++){
						BizObject provider = providers.get(i);
						extobj2[i] = new DocExtClass();
						extobj2[i].setAttr1(provider.getAttribute("CustomerName").getString());
				...
				if(salers.size()<=5){
					...
					for(int i=0;i<salers.size();i++){
						...
						extobj3[i].setAttr5(DataConvert.toMoney(saler.getAttribute("InvestmentSum").getDouble()/10000));
						extobj3[i].setAttr6(saler.getAttribute("WHETHEN1").getString());
					}
				}else if(salers.size()>5){
					extobj3 = new DocExtClass[5];
					for(int i=0;i<5;i++){
						BizObject saler = salers.get(i);
						extobj3[i] = new DocExtClass();
						extobj3[i].setAttr1(saler.getAttribute("CustomerName").getString());
