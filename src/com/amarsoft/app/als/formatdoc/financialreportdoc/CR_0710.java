...
public class CR_0710 extends FormatDocData implements Serializable{
	...
	public boolean initObjectForRead() {
		ARE.getLog().trace("CR_0710.initObject()");
		...
		try{
			...
			BizObject bo = null;
			Map reportMap = null;
			String sCustomerID=getCustomerMsg(sObjectNo);
			FinanceDataManager fdm=new FinanceDataManager();
			if(!StringX.isSpace(sCustomerID)){
				CustomerFSRecord cfsr=fdm.getNewestReport(sCustomerID);//最新一期报表
				...
				double olajamount = Double.parseDouble(getTotalData(lastReportNo, "02"));
...
//							extobj1[i].setAttr3("0");
//						}else{
//							extobj1[i].setAttr3(String.format("%.0f",(fdr.getAmount()/yfzk)*100));//与本企业关系暂时没有获得
//						}
//					}
//					totals = DataConvert.toMoney(tota);
//				}
...
//						extobj2[i] = new DocExtClass();
//						FinanceDetailRcv fdr = rcvList.get(i);
//						tota1 = fdr.getAmount() + tota1;
//						extobj2[i].setAttr0(fdr.getDataType());
//						extobj2[i].setAttr1(fdr.getDataName());
//						if(cunhuo==0){
//							extobj2[i].setAttr2("0");
//						}else{
//							extobj2[i].setAttr2(String.format("%.0f",(fdr.getAmount()/cunhuo)*100));
//						}
				...
				if(assets.size()>0){
					for(int i=0;i<assets.size();i++){
						...
						if(gdzc==0){
							...
							extobj3[i].setAttr3(String.format("%.0f",(dd/gdzc)*100));
						}
